...
def build_excel_report(rec: dict[str, Any]) -> bytes:
    ...
    ws["A2"] = (f"{display_period(rec.get('period1'), l1)} → {display_period(rec.get('period2'), l2)}"
                f"   ·   Currency: {cur or '-'}   ·   Generated by ACE Audit AI")
    ws["A2"].font = LABEL_FONT

    _header(ws, 4, ["Key figure", l1, l2, "Change", "Change %"])
    rows = [("Total income", "total_income", True), ("Total expense", "total_expense", False),
    ...
    ws.cell(r, 1, "Profit margin").font = LABEL_FONT
    ...
    r += 2
    ws.cell(r, 1, "Alerts").font = LABEL_FONT
    ws.cell(r, 2, f"{counts['error']} critical · {counts['warning']} warnings · {counts['info']} notes")

    r += 2
    ws.cell(r, 1, "CEO summary").font = TITLE_FONT
    r += 1
    for line in _strip_markdown(rec.get("summary") or "(no summary)").splitlines():
        if line.strip():
    ...
    _header(ws, 1, ["Item", l1, l2, "Net Variance", "Combined", "Change %"])
    for i, t in enumerate(m.get("totals") or [], start=2):
        ws.cell(i, 1, t["Item"])
        ws.cell(i, 2, _num(t.get(l1))).number_format = MONEY
        ws.cell(i, 3, _num(t.get(l2))).number_format = MONEY
        ws.cell(i, 4, _num(t.get("Net Variance"))).number_format = SIGNED_MONEY
        ws.cell(i, 5, _num(t.get("Combined"))).number_format = MONEY
        ws.cell(i, 6, _num(t.get("Change %"))).number_format = PCT
    _autosize(ws)

    # ---- Ratios ------------------------------------------------------------
    ws = wb.create_sheet("Ratios")
    _header(ws, 1, ["Metric", l1, l2])
    for i, rt in enumerate(m.get("ratios") or [], start=2):
        fmt = MULT if rt.get("unit") == "x" else PLAIN_PCT
        ws.cell(i, 1, rt["Metric"])
    ...
    for sheet, key, good_up in (("Income", "income_by_category", True),
        ...
        _header(ws, 1, ["Category", l1, l2, "Difference", "Change %"])
        rows_ = m.get(key) or []
    ...
    _header(ws, 1, ["Severity", "Message"])
    label = {"error": "Critical", "warning": "Warning", "info": "Note"}
    for i, a in enumerate(alerts, start=2):
        sev = a.get("severity", "info")
        ws.cell(i, 1, label.get(sev, sev)).fill = SEVERITY_FILL.get(sev, SEVERITY_FILL["info"])
        c = ws.cell(i, 2, a.get("message"))
        c.alignment = Alignment(wrap_text=True, vertical="top")
